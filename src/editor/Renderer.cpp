...
void Renderer::pickObject()
{
	...
	auto tmpPickEnt = tmpPickInfo.GetSelectedEnts();
	...
	else
	{
		...
		{
			pickCount++;
			selectEnt(SelectedMap, tmpPickEnt.size() ? (int)tmpPickEnt[0] : -1, anyCtrlPressed);
		}
	}
}
...
{
	Bsp* map = SelectedMap;
	auto pickEnts = pickInfo.GetSelectedEnts();
